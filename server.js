...
const PORT = 3000;
const BRIDGE_HOST = '192.168.2.111';
...
const server = http.createServer(async (req, res) => {
    ...
    req.on('end', () => {
      try {
        ...
        const aReq = require('http').request({
          hostname: '192.168.2.111', port: 18800, path: '/api/notify', method: 'POST',
  ...
  if (url.pathname === '/api/audit-log' && req.method === 'GET') {
    try {
      const log = fs.existsSync('/var/log/n1-audit.log')
        ? fs.readFileSync('/var/log/n1-audit.log', 'utf8')
        : '';
      const lines = log.trim().split('\n').filter(Boolean).reverse().slice(0, 200);
      res.writeHead(200, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify({ lines }));
    } catch(e) {
      res.writeHead(500); res.end(JSON.stringify({ lines: [] }));
    }
    return;
  }
  ...
  if (url.pathname === '/api/smb-mount' && req.method === 'POST') {
    ...
    req.on('end', () => {
      try {
        ...
        const files = fs.readdirSync(mountPoint).reduce((arr, f) => {
          try {
            const st = fs.statSync(path.join(mountPoint, f));
            arr.push({ name: f, size: st.size, isDir: st.isDirectory() });
          } catch(e) {}
          return arr;
        }, []);
        res.writeHead(200, { 'Content-Type': 'application/json' });
