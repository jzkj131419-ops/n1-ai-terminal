...
### 铁律1：条数对
```
补录后附表非空行数 == 补录前条数 + 确认补录行数
```
周贤民生案：4,660 + 184 = **4,844** ✓

### 铁律2：净额方向合理
```
新净额 = 旧净额 + 补录收入合计 - 补录支出合计
```
周贤民生案：-1,388,664 + 1,924,080 - 1,329,569 ≈ **-794,153** ✓  
（注：旧净额为附近值，精确算以脚本输出为准）
...
import openpyxl
from decimal import Decimal
wb = openpyxl.load_workbook('周贤_民生_审计附表.xlsx')
ws = wb['1交易记录汇总']
inc = exp = Decimal('0'); cnt = 0; new_cnt = 0
for r in ws.iter_rows(min_row=2, values_only=True):
    if not r[0]: continue
    cnt += 1
    if r[7]: inc += Decimal(str(r[7]))
    if r[8]: exp += Decimal(str(r[8]))
    if r[15] == '[漏录补录]': new_cnt += 1
print(f'总条数:{cnt}  其中补录:{new_cnt}')
print(f'收入:{inc:,.2f}  支出:{exp:,.2f}  净额:{inc-exp:,.2f}')
"
```

### 铁律3：差额<0行的处理说明（重要）

核查清单中差额<0表示"该日期/方向，附表已有的合计金额 > PDF单笔金额"。  
若用户确认全部补录（`--confirm-all`），这些行也会写入附表，导致：
- 附表该日期条数 > PDF该日期条数
- 净额变化比预估更大

**这不是错误**，前提是用户已明确知晓并确认。必须在交付文件备注中注明：  
`"本版附表含184条人工确认补录，其中104条差额为负，附表已多于PDF对应日期合计。"`

---

## 八、今日实操原始命令（周贤民生 2026-06-01）

### Step 1：补录写入附表 + 总台账

```bash
# 脚本路径（本次临时生成）
python3 /tmp/mingsheng_buhu.py
```

脚本逻辑（已归档在本手册第四节 `supplement_to_xlsx.py`，本次等价于 `--confirm-all`）：
1. 读 `民生_漏录核查清单.xlsx` 184行
2. 映射字段：日期/方向→D|C/金额/余额/摘要/对手方 → 附表16列格式
3. 追加到 `1交易记录汇总`，按日期重排序
4. 同步写入 `总台账底稿.csv`，按银行+日期排序
5. 打印收支合计供核验
...
| 方向=支出 | 交易方向=D | 方向=收入→C |
| PDF金额 | 交易金额/收入金额/支出金额 | 按方向分拆 |
| PDF余额 | 交易余额 | |
| PDF摘要 | 摘要名称 | |
| PDF对手方 | 对方客户名称 | |
| （固定值） | 交易时间=00:00:00 | PDF无时间 |
| （固定值） | 备注=[漏录补录] | 橙色高亮对手账号列 |
...
# 修改头部三个常量：
# BASE = "/Volumes/AI审计/.../03_民生"
# CUSTOMER = "周贤"
# ACCOUNT  = "50000000000414464745（卡号 6216910103670714）"
# BANK     = "中国民生银行"
...
输出：`周贤_民生_审计报告.docx` + `.md` + 附表分析 sheet4~17 全刷新

### Step 3：更新 00_汇总分析.md

手动替换以下字段（从报告第二节取数）：

```
民生笔数:   4,660 → 4,844
民生收入:   ¥29,685,284.29 → ¥31,789,364.37
民生支出:   ¥31,240,659.13 → ¥32,570,228.47
民生净额:   ¥-1,555,374.84 → ¥-780,864.10
三账户总收: ¥35,732,302.36 → ¥37,656,382.44
三账户总支: ¥37,120,966.81 → ¥38,450,536.15
三账户净额: ¥-1,388,664.45 → ¥-794,153.71
报告日期:   2026-05-31 → 2026-06-01
```
...
| — 差额<0（橙，附表多） | 104行 |
| — 差额=0（绿，无净差） | 20行 |
| 用户确认方式 | 口头确认全部184条补录（--confirm-all） |
| 补录后附表条数 | 4,844（+184） |
| 补录新增收入 | ¥1,924,080.08 |
| 补录新增支出 | ¥1,329,569.34 |
| 补录后民生净额 | ¥-780,864.10 |
| 三账户净额（补录后） | ¥-794,153.71 |
| 附表备份文件 | 周贤_民生_审计附表_pre_buhu.xlsx |
| 总台账备份文件 | 总台账底稿_pre_buhu.csv |
| 总台账更新后条数 | 19,775（原19,591） |
| 报告生成路径 | 03_民生/周贤_民生_审计报告.docx/.md |
